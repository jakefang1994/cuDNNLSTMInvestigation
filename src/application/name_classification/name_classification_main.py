...
def generate_data(batch_size, sequence_length,input_size):
    ...
    batch_data = Variable(torch.from_numpy(batch_data)).cuda()
...
def train():
    ...
    rnn.zero_grad()
    data, target, category = generate_data(10, 20, 57)
    output = rnn(data)
    ...
    return output, loss.data[0], category, rnn

def evaluate():
    data, target, category = generate_data(1, 20, 57)
    output = rnn(data)
...
if __name__ == "__main__":
    ...
    rnn = LSTM(input_size, hidden_size, num_layers, n_categories)
    rnn.cuda()
    ...
    for iter in range(1, n_iters + 1):
        output, loss, category, rnn = train()
        current_loss += loss
